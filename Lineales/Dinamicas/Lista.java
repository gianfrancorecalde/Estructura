...
public class Lista {
    ...
    // Método Clone
    public Lista clone(){ 
        // este metodo clone esta mal xq uso un metodo del TDA
        Lista clone = new Lista();
        Nodo recorrido = this.cabecera;
        int i;
        if(!this.esVacia()){
            for(i=1; i<=this.longitud(); i++){
                clone.insertar(recorrido.getElemento(), i);
                recorrido = recorrido.getEnlace();
            }
        }
        return clone;
    ...
    public String toString(){
        ...
        if(!this.esVacia()){
            Nodo recorrido = this.cabecera;
            for(i=1; i<=this.longitud();i++){
                cadena = cadena + " "+ recorrido.getElemento();
                recorrido = recorrido.getEnlace();
            }
